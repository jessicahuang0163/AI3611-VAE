...
def set_wandb(dir_name, variant):
    wandb.init(
        ...
        group=variant.get("pipeline_exp_name", None),
        settings=wandb.Settings(start_method="fork"),
    )


def set_snapshot_dir(dir_name, variant, log_wandb=True):
    global _snapshot_dir, _log_wandb
...
def save_torch_model(model, name="model.ckpt"):
    file_name = osp.join(_snapshot_dir, name)
